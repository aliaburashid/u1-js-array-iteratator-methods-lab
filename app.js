...
// Complete the exercise in the space below:
// if a - b gives: negatives then (a comes before b ), positives then (b comes before a), zero then (keep the same)
...
// Complete the exercise in the space below:
inventorNamedAda = inventors.find((person) => {
...
firstLast = people.map((person) => {
    const divideIntoParts = person.split (', ');
    return `${divideIntoParts[1]} ${divideIntoParts[0]}`;
});
